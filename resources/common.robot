...
Set Date Field
    ...
    Wait Until Element Is Visible    id=${field_id}    timeout=10s
    Wait Until Element Is Enabled    id=${field_id}    timeout=10s
    Execute Javascript    document.getElementById('${field_id}').value = '${date_value}'; document.getElementById('${field_id}').dispatchEvent(new Event('input', {bubbles:true})); document.getElementById('${field_id}').dispatchEvent(new Event('change', {bubbles:true}));
